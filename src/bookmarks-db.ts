...
export const ALLOWED_BOOKMARK_STATS_COLUMNS = [
...
export type AllowedBookmarkStatsColumn = (typeof ALLOWED_BOOKMARK_STATS_COLUMNS)[number];

export function assertAllowedBookmarkStatsColumn(value: string): AllowedBookmarkStatsColumn {

  if ((ALLOWED_BOOKMARK_STATS_COLUMNS as readonly string[]).includes(value)) {

    return value as AllowedBookmarkStatsColumn;

  }

  throw new Error(`Unsupported bookmark stats column: ${value}`);
...
export async function getFieldCounts(column: AllowedBookmarkStatsColumn): Promise<Record<string, number>> {
  const safeColumn = assertAllowedBookmarkStatsColumn(column);
  const dbPath = twitterBookmarksIndexPath();
  const db = await openDb(dbPath);
  ensureMigrations(db);
  try {
    const rows = db.exec(
      `SELECT ${safeColumn}, COUNT(*) as c FROM bookmarks
       WHERE ${safeColumn} IS NOT NULL
       GROUP BY ${safeColumn} ORDER BY c DESC`
    );
...
export async function sampleByField(
  column: AllowedBookmarkStatsColumn,
  value: string,
  limit: number,
